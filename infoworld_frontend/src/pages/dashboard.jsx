...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-cyan-50 text-cyan-900 flex flex-col">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-10 py-20 flex flex-col md:flex-row gap-20">
        {/* Left Sidebar - Profile + Details */}
        <div className="md:flex flex-col w-[1000px] bg-cyan-100 rounded-3xl p-12 shadow-xl border border-cyan-300 items-center">
          <img
            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(
              dashboardData.name
            )}&background=06b6d4&color=ffffff&size=160`}
            alt="Profile"
            className="rounded-full w-40 h-40 mb-8 shadow-lg border-8 border-cyan-500 object-cover"
          />
          {/* Personal details below profile photo */}
          <div className="w-full mt-6 text-left text-lg leading-relaxed space-y-4 text-cyan-800">
            <h2 className="text-3xl font-bold text-cyan-700 border-b-4 border-cyan-500 pb-3 mb-6">
              Personal Info
            </h2>
        ...
        <section className="flex-grow bg-white rounded-3xl shadow-xl p-10 flex flex-col">
          <h2 className="text-3xl font-extrabold text-cyan-700 mb-6 border-b-2 border-cyan-400 pb-2">
            {user.accountType === "reporter"
              ? "News You've Uploaded"
              : "News You've Liked or Commented"}
          </h2>
          {uploadedNews.length === 0 ? (
            <p className="text-cyan-600 italic text-lg">No news to show...</p>
          ) : (
            <ul className="space-y-8 max-h-[480px] pr-4 overflow-y-auto hide-scrollbar">
              {uploadedNews.map((n) => (
                <li
                  key={n._id}
                  className="border rounded-2xl border-cyan-300 p-6 shadow-md hover:shadow-cyan-400/40 transition-shadow cursor-pointer"
                  title={n.title}
                >
                  <h3 className="text-xl font-semibold text-cyan-900 mb-2">{n.title}</h3>
                  <p className="text-sm text-cyan-700 line-clamp-3">{n.content}</p>
